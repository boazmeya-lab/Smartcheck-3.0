...
// Vérification stricte du SCAN PHYSIQUE uniquement (ignore la colonne RSVP)
function verifierPresence(guest) {
    if (!guest) return false;
    
    if (guest.scan_effectue === true) return true;
    
    let scanVal = guest.scan_status || guest.statut_scan || guest.scanne;
    ...
    if (invite) {
        ...
        declencherFlash(
            `<div class="flash-title">Accès Refusé ❌</div>
             ...
             <div class="flash-extra">Veuillez synchroniser la base de données dans l'onglet "Invités".</div>`, 
            'var(--danger)'
        );
        relancerScanneurApresDelai();
    }
